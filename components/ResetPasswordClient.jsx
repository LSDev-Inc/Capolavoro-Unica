  ...
  return (
    <main className="min-h-screen px-6 py-12">
      <div className="mx-auto flex w-full max-w-md flex-col gap-6">
        ...
        <h1 className="font-display text-3xl text-white">{t("reset.title")}</h1>
        <p className="mt-2 text-sm text-muted">{t("reset.subtitle")}</p>

        {missing ? (
          <p className="mt-6 text-sm text-red-300">{t("reset.missing")}</p>
        ) : (
          <form className="mt-8 flex flex-col gap-4" onSubmit={handleSubmit}>
            ...
            >
              {loading ? t("reset.buttonLoading") : t("reset.button")}
            </button>
          </form>
        )}
        </div>
